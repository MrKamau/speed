...
public class GaugeSpeed extends View {
    ...
    public static final boolean SHOW_OUTER_SHADOW = false;
    ...
    public static final float NEEDLE_HEIGHT = 0.35f;
    ...
    public void setFace(int mode)
    {
        if(mode == 0)
        {
            faceTexture = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.plasticblack);
        }
        else if (mode == 1)
        {
            faceTexture = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.plasticblue);
        }
        else if (mode == 2)
        {
            faceTexture = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.plasticred);
        }
        else if (mode == 3)
        {
            faceTexture = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.plasticgreen);
        }
        else if (mode == 4)
        {
            faceTexture = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.plasticyellow);
        }

        BitmapShader paperShader = new BitmapShader(faceTexture,
                Shader.TileMode.MIRROR,
                Shader.TileMode.MIRROR);
        mFacePaint.setShader(paperShader);
    }
    ...
    public Paint getDefaultFaceShadowPaint() {
        final Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setShader(new RadialGradient(0.5f, 0.5f, mFaceRect.width() / 2.0f, new int[]{Color.argb(60, 40, 96, 96),
                Color.argb(80, 15, 34, 34), Color.argb(120, 0, 0, 0), Color.argb(140, 0, 0, 0)},
